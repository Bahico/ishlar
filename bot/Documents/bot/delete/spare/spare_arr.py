...
async def bot_del_spare(message: types.Message, conn: sqlite3.Connection, vote_cb: aiogram.utils.callback_data.CallbackData, callback_query: dict):
    """
    ...
    """
    spare = conn.execute(f"select spare_name, number from SPARE where id = (?) and tur = (?);",
                         (callback_query["name"], callback_query["stage"])).fetchall()
    markup = InlineKeyboardMarkup(resize_keyboard=True)
    markup.row_width = 3
    for i in spare:
        markup.insert(InlineKeyboardButton(text=i[0],
                                           callback_data=vote_cb.new(stage="spare_arr", id="del_menu", name=i[1],
                                                                     number=callback_query["stage"],
                                                                     language=callback_query["language"],
                                                                     action="bot")))
    if callback_query["language"] == "uzb":
        markup.insert(InlineKeyboardButton(text="Hammasi❌", callback_data=vote_cb.new(stage="spare_arr", id="del_menu",
                                                                                      name=str(callback_query[
                                                                                                   "stage"]) + "," + str(
                                                                                          callback_query["name"]),
                                                                                      number="all",
                                                                                      language=callback_query[
                                                                                          "language"], action="bot")))
        markup.insert(InlineKeyboardButton(text="⬅️Ortga", callback_data=vote_cb.new(stage="year", id="del_menu",
                                                                                     name=callback_query["name"],
                                                                                     number="", language=callback_query[
                "language"], action="bot")))
