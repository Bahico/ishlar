...
async def bot_del_car_name(message: types.Message, conn: sqlite3.Connection, vote_cb: aiogram.utils.callback_data.CallbackData, callback_query: dict, error=None):
    """
    ...
    for i in car_set:
        if i is not None and i != "bypass":
            markup.insert(InlineKeyboardButton(text=i, callback_data=vote_cb.new(stage="name", id="del_menu", name=i,
                                                                                 number="None",
    ...
    if callback_query["language"] == "uzb":
        markup.row(
            ...
            InlineKeyboardButton(text="⬅️Ortga",
                                 callback_data=vote_cb.new(stage="spare_del", id="main_menu", name="back",
                                                           number="None", language=callback_query["language"],
                                                           action="bot")))
        if error is None:
            markup.add(InlineKeyboardButton(text="↪️Barcha modellarga",
                                            callback_data=vote_cb.new(stage="name", id="del_menu", name="bypass",
            ...
            await bot.send_message(message.from_user.id, "Hozirda \"↪️Barcha modellarga\" extiyot qism yo'q",
                                   reply_markup=markup)
    ...
    if i:
        conn.execute(f"update I set corporation = '{callback_query['name']}' where id = (?);", (message.from_user.id,))
    else:
        conn.execute("insert into I(id,corporation) values (?,?);", (message.from_user.id, callback_query["name"]))


async def bot_del_name_bypass(message: types.Message, conn, vote_cb, callback_query):
    car = conn.execute("select id from CAR where corporation = (?) and name = 'bypass';",
                       (callback_query["number"],)).fetchall()
    if car:
