...
async def client_spare_number(message: types.Message, conn: sqlite3.Connection,
    ...
    car = conn.execute(f"select spare_name, number from SPARE where id = (?) and tur = '{callback_query['stage']}';",
    ...
    for i in car:
        ...
        markup.add(InlineKeyboardButton(text=i[0],
                                        callback_data=vote_cb.new(stage=callback_query["stage"], id="spare", name=i[1],
