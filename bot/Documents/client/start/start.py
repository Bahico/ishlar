...
async def client_start(message: types.Message, conn: sqlite3.Connection, vote_cb: aiogram.utils.callback_data.CallbackData):
    branch = conn.execute("select language, branch from ADMIN where id = (?) and test = 4;",
    ...
    if I_:
        ...
        markup.row(
            ...
            InlineKeyboardButton(text="🇬🇧 English",
                                 callback_data=vote_cb.new(stage="start", id="start", name="None", number="None",
                                                           language="eng", action="client")),
            InlineKeyboardButton(text="🇷🇺 Русский",
                                 callback_data=vote_cb.new(stage="start", id="start", name="None", number="None",
                                                           language="rus", action="client"))
        )
