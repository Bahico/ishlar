...
from ..bypass.bypass import branch_name_bypass
...
from ..product.product_plus import admin_plus_name, admin_plus_corporation, admin_plus_brand
from ..product.new.new_name import admin_new_name
...
async def admin_branch_callback(message: types.Message, conn: sqlite3.Connection, vote_cb: aiogram.utils.callback_data.CallbackData, callback_query: dict):
    """
    ...
    if callback_query["id"] == "start":
        ...
        if callback_query["stage"] == "admins":
            ...
            await admin_adit_language(message, vote_cb, callback_query)
        ...
        await admin_language_edit(message, conn, callback_query)
