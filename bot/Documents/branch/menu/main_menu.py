...
async def admin_branch_menu(message: types.Message, conn: sqlite3.Connection, vote_cb: aiogram.utils.callback_data.CallbackData, language: str):
    conn.execute("update ADMIN set settings = '0' where id = (?);", (message.from_user.id,))
    if language == "uzb":
        markup = InlineKeyboardMarkup(resize_keyboard=True)
        markup.row(
            InlineKeyboardButton(text="🛒 Maxsulotlar",
                                 callback_data=vote_cb.new(stage="product", id="menu", name="None", number="branch",
                                                           language=language, action="branch")),
            InlineKeyboardButton(text="⚙️ Filial sozlamalari",
                                 callback_data=vote_cb.new(stage="settings", id="menu", name="None", number="branch",
                                                           language=language, action="branch"))
        )
        markup.row(
            InlineKeyboardButton(text="🙍🏻 Odamlar",
                                 callback_data=vote_cb.new(stage="people", id="menu", name="None", number="branch",
                                                           language=language, action="branch")),
            InlineKeyboardButton(text="🗺 Filial joylashuvi",
                                 callback_data=vote_cb.new(stage="location", id="menu", name="None", number="branch",
                                                           language=language, action="branch"))
        )
        markup.add(InlineKeyboardButton(text="🛢 Mashina yog'i",
                                        callback_data=vote_cb.new(stage="oil", id="menu", name="None", number="branch",
                                                                  language=language, action="branch")))
