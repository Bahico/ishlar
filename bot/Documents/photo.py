...
async def get_photo(message: types.Message, conn: sqlite3.Connection, vote_cb: aiogram.utils.callback_data.CallbackData):
    ...
    if admin and admin[0][0].split(",")[0] == 'spare photo':
        conn.execute("insert into PHOTO(id,type,photo) values (?,'spare',?);",
                     (int(admin_[0][0]), message.photo[0].file_id))
