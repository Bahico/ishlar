...
async def admin_start_admin(message: types.Message, conn, vote_cb):
    person = conn.execute("select * from ADMIN where id = (?);",(message.from_user.id,)).fetchall()
    if not person:
        markup = InlineKeyboardMarkup(resize_keyboard=True)
        markup.row_width = 2
        markup.row(
            InlineKeyboardButton(text="🇺🇿 O'zbekcha",callback_data=vote_cb.new(stage="start", id="start", name="None", number="None",language="uzb", action="branch")),
            InlineKeyboardButton(text="🇬🇧 English",callback_data=vote_cb.new(stage="start", id="start", name="None", number="None",language="eng", action="branch")),
            InlineKeyboardButton(text="🇷🇺 Русский",callback_data=vote_cb.new(stage="start", id="start", name="None", number="None",language="rus", action="branch"))
        )
        await bot.send_message(message.from_user.id, "Tilni tanlang 😊\nSelect a language 😊\nВыберите язык 😊",reply_markup=markup)


async def admin_login(message: types.Message, conn, language):
    if language == "uzb":
