...
from ..basket.reduce import \
    ...
    client_reduce_reduce, client_reduce_send, client_reduce_arrived, client_reduce_info
from ..car_arr.car_year import client_car_year, client_year_bypass
from ..car_arr.car_name import client_car_name, client_name_bypass
from ..car_arr.car_number import client_car_number, client_number_bypass
from ..car_arr.car_pazitsiya import client_car_brand, client_paditsiya_bypass
...
async def client_client_callback(message: types.Message, conn, vote_cb, callback_query):
    if callback_query["id"] == "start":
        if callback_query["stage"] == "start":
            ...
            if callback_query["name"] == "bypass":
                ...
                await client_car_number(message, conn, vote_cb, callback_query)

        elif callback_query["stage"] == "car_number":
            if callback_query["name"] == "bypass":
                await client_number_bypass(message, conn, vote_cb, callback_query)
            else:
                await client_car_brand(message, conn, vote_cb, callback_query)

        elif callback_query["stage"] == "car_brand":
            if callback_query["name"] == "bypass":
                await client_paditsiya_bypass(message, conn, vote_cb, callback_query)
            else:
                await client_car_year(message, conn, vote_cb, callback_query)

        elif callback_query["stage"] == "car_year":
            if callback_query["name"] == "bypass":
                await client_year_bypass(message, conn, vote_cb, callback_query)
            else:
