...
async def get_message_all(message: types.Message, conn, vote_cb):
    ...
    bot_ = conn.execute("select settings, language, name, login from I_ where id = (?);",(message.from_user.id,)).fetchall()
    if bot_:
